...
# python D3R_GC2_ranking_calculation.py --submitdir ./example_free_energy_submission_folder --templatefile ./experimental_data/experimental_affinity.csv --freeenergy  

### Do not use freeenergy option to calculate the scoring type of submissions ###
# python D3R_GC2_ranking_calculation.py --submitdir ./example_scoring_submission_folder --templatefile ./experimental_data/experimental_affinity.csv   
########################
...
def calculate_kendalls (template_value, submitted_value, boot_bins = 10000, exp_unc = 0):
...
def extract_ranking_list(template_csv, submitted_csv, system_name = "FXR_", method = "score"):
    ...
    for template_line in template_lines:
        ...
        if method == "fep":
            ...
            template_value = float(template_line.split(",")[1])
        ...
        if system_name in submitted_line: 
            ...
            if submitted_title in template_dic:
                try:
                    ...
                    pass
    return (len(title_list), template_value_list, submitted_value_list)   
   

def main_ranking (submitted_folder_path, template_affinity_file, bootstrap = 10000, free_energy = True):
    if free_energy:
        ...
        try:
            num_bins, template_value_list, submitted_value_list = extract_ranking_list(template_affinity_file, submitted_csv, system_name = "FXR_", method = "fep")
            logging.info("Got %s of submitted ligand in this submission"%num_bins)
            rms_error, rms_errors_std, tau, taus_std, spearman, spearmans_std, Pearson, Pearsons_std = calculate_kendalls (template_value_list, submitted_value_list, boot_bins = bootstrap)
        ...
        try:
            ...
            num_bins, template_value_list, submitted_value_list = extract_ranking_list(template_affinity_file, submitted_csv, system_name = "FXR_")
            logging.info("Got %s of submitted ligand in this submission"%num_bins)
            rms_error, rms_errors_std, tau, taus_std, spearman, spearmans_std, Pearson, Pearsons_std = calculate_kendalls (template_value_list, submitted_value_list, boot_bins = bootstrap)
            logging.info("The Kendall's Tau value for this submission is %s"%tau)
...
if ("__main__") == (__name__):
    ...
    main_ranking (submitDir, tempfile, bootstrap = bootstrap_bins , free_energy = free_energy_option)
