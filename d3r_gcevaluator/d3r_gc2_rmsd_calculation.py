...
def wait_and_check (filename, timestep = 5, how_many_times = 100):
    ...
    while (count < how_many_times):
        if not os.path.isfile(filename):
            time.sleep(timestep)
            print "Inside the wait loop check the current wait time:%s "%(count*timestep)
            count = count + 1
        else:
            return True
    return False
...
def align_protein (template_complex, input_complex, output_complex):
    ...
    return commands.getoutput("$SCHRODINGER/utilities/align_binding_sites %s %s -o %s"%(template_complex, input_complex, output_complex))
...
def rmsd_calculation(input_ligand, template_ligand, input_protein, template_protein_complex, realignment = False ):
    ...
    if realignment:
        ...
        try:
            ...
            time_check_frequence = 5
            how_many_check_point = 100
